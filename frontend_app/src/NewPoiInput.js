...
class NewPoiInput extends Component {
    ...
    handleErrorClass = (e) => {
        ...
        this.setState(newErrorObj, () => {
            this.formVerificationAndSubmit();
        });
    };
    ...
    componentDidMount() {
        if (this.props.view === 'edit') {
            ...
            let editPoi = this.props.pois.find(
                (poi) => poi.id === parseInt(this.props.match.params.poiId)
            );
            console.log(editPoi);
    ...
    render() {
        ...
        const amenityListOptions = amenityList.sort().map((singleAmenity) => {
            return (
                <DropdownItem eventKey={singleAmenity} key={singleAmenity}>
                    {singleAmenity}
                </DropdownItem>
            );
        });
                ...
                <Form.Group>
                    <Row>
                        <Col md={6} sm={12}>
                            ...
                            <Form.Control
                                className={
                                    this.state.streetError ? 'inputError' : ''
                                }
                                type="text"
                                id="street"
                                placeholder="Street"
                        ...
                        <Col md={6} sm={12}>
                            ...
                            <Form.Control
                                className={
                                    this.state.latError ? 'inputError' : ''
                                }
                                type="number"
                                placeholder="lat"
                                id="lat"
                            ...
                            <Form.Control
                                className={
                                    this.state.lonError ? 'inputError' : ''
                                }
                                type="number"
                                placeholder="lon"
                                id="lon"
                            ...
                            <div className="d-flex">
                                <Form.Control
                                    ...
                                    id="amenity"
                                    className={
                                        this.state.amenityError
                                            ? 'inputError readOnlyAmenity'
                                            : 'readOnlyAmenity'
                                    }
                                    readOnly
...
const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        changePoiInfo: (e) => {
            dispatch(changePoiInfo(e));
        },
        changeAmenity: (newAmenity) => {
            dispatch(changeAmenity(newAmenity));
        },
        selectPoiForEdit: (poiObj) => {
            dispatch(selectPoiForEdit(poiObj));
        },
        clearNewPoiInfoState: () => {
            dispatch(clearNewPoiInfoState());
        },
        addNewPoi: (newPoi) => {
            dispatch(addNewPoi(newPoi, ownProps));
        },
        updatePoi: (updatedPoi) => {
            dispatch(updatePoi(updatedPoi, ownProps));
        },
    };
};
